...
# Moved to run_host_shim.py so steam_finder can use them without a circular
# import; re-exported because other modules import them from here.
from Utils.wine_proton.run_host_shim import (  # noqa: F401
    _apply_run_host_shim,
    _needs_run_host_shim,
    _run_host_shim_prefix,
)
...
WINEPREFIX + Proton's bin on PATH, ``wine start.exe <exe>``), with only two
    ...
    *extra_env* is applied last, a value of ``None`` removes the variable.
    ...
    if extra_args:
        cmd = cmd + list(extra_args)
    cmd = _apply_run_host_shim(cmd, Path(env["WINEPREFIX"]), label, log_fn)
